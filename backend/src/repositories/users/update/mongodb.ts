...
import { type IUpdateUserRepository } from '../../../controllers/users/update/protocols'
...
export class MongoUpdateUserRepository implements IUpdateUserRepository {
  async updateUser(body: UpdateUserParamsType, params: { id: string }): Promise<UserType> {
    const canUpdateUser = await new MongoQueryDbRepository().canUpdateUser(
      body.name,
      body.email,
      params.id,
    )
        ...
        $set: {
          ...
          credentials: {
            ...
            password: body.password
              ...
              : defualtUser.credentials.password,
          },
        },
      },
    )
